...
public final class ConsolePrompt {
    ...
    public static String promptText(String label) {
        while (true) {
            System.out.print(ConsoleTheme.BRAND_GREEN + " > " + ConsoleTheme.RESET + label + ": ");
            String input = getReader().readLine().trim();
            if (!input.isEmpty()) {
                return input;
            }
            System.out.println(ConsoleTheme.error("   Input cannot be empty. Please try again."));
        }
    }
    ...
    public static String promptPassword(String label) {
        while (true) {
            ...
            System.out.println(ConsoleTheme.error("   Password cannot be empty."));
        }
    }
        ...
        while (true) {
            ...
            try {
                ...
                if (amount.compareTo(BigDecimal.ZERO) <= 0) {
                    System.out.println(ConsoleTheme.error("   Amount must be greater than zero."));
                    continue;
                }
                return amount;
            } catch (NumberFormatException e) {
                System.out.println(ConsoleTheme.error("   Invalid numeric amount format. (e.g., 250.00)"));
            }
        }
    }
    ...
    public static BigDecimal promptAmountOptional(String label, BigDecimal defaultValue) {
        while (true) {
            ...
            try {
                BigDecimal amount = new BigDecimal(input);
                if (amount.compareTo(BigDecimal.ZERO) < 0) {
                    System.out.println(ConsoleTheme.error("   Amount cannot be negative."));
                    continue;
                }
                return amount;
            } catch (NumberFormatException e) {
    ...
    public static boolean promptConfirmation(String message) {
        System.out.print(ConsoleTheme.warning(" ? " + message + " [y/N]: "));
    ...
    public static void pause(String message) {
        System.out.println();
        System.out.print(ConsoleTheme.muted(" " + message));
        getReader().readLine();
    }
}